...
# Ticket system storage (in-memory to minimize storage usage)
active_tickets: Dict[int, Dict] = {}  # channel_id -> {ticket_id, user_id, category, created_at, guild_id}
ticket_counter = 1000  # Starting ticket ID
...
class TicketSystem:
    ...
    def create_ticket_buttons():
        ...
        # Partnership/Business Support button
        partnership_button = discord.ui.Button(
            label="Partnership/Business Support",
            emoji="🤝",
    ...
    @staticmethod
    async def create_ticket_channel(guild, user, category):
        ...
        try:
            channel = await guild.create_text_channel(
                ...
                category=None,
                topic=f"Ticket #{ticket_counter} - {category} - {user.display_name}"
            )
...
async def help_command(ctx):
    ...
    embed.add_field(
        name="🎫 Ticket System",
        value="• Use `!ticket` to create support panel\n• Click buttons to create tickets\n• Categories: Partnership, Purchase, General\n• Use `!close` to close tickets",
        inline=False
    )
...
async def close_ticket_command(ctx):
    ...
    if ctx.author.id != ticket_data['user_id'] and not ctx.author.guild_permissions.manage_channels:
    ...
    embed = discord.Embed(
        title="🔒 Ticket Closed",
        description=f"Ticket #{ticket_data['ticket_id']} has been closed by {ctx.author.mention}",
        color=0xff0000
    )
    embed.add_field(name="Category", value=ticket_data['category'], inline=True)
    embed.add_field(name="Created", value=ticket_data['created_at'].strftime("%Y-%m-%d %H:%M:%S"), inline=True)
...
async def on_interaction(interaction):
    ...
    if custom_id and custom_id.startswith('ticket_'):
        ...
        category_map = {
            'ticket_partnership': 'Partnership/Business Support',
            'ticket_purchase': 'Purchase',
            'ticket_general': 'General Support'
        }
        
        category = category_map.get(custom_id)
        if not category:
        ...
        for ticket_data in active_tickets.values():
            if ticket_data['user_id'] == interaction.user.id and ticket_data['guild_id'] == interaction.guild_id:
        ...
        # Create ticket channel
        channel, ticket_id = await TicketSystem.create_ticket_channel(interaction.guild, interaction.user, category)
        ...
        if channel:
            ...
            embed = discord.Embed(
                ...
                description=f"Hello {interaction.user.mention}! Thank you for contacting RDM support.\n\nPlease describe your issue in detail and our support team will assist you shortly.",
                color=0x5865F2
            )
            embed.add_field(name="Category", value=category, inline=True)
            embed.add_field(name="Created by", value=interaction.user.mention, inline=True)
            embed.add_field(name="How to close", value="Type `!close` to close this ticket", inline=False)
            embed.set_footer(text="RDM Support Team")
